...
while count < num:
  ...
  leftItemNum = len(indexList)#indexList的长度是5
  ...
  nextnodeList = sample(indexList, leftItemNum)#random.sample(sequence, k)，从指定序列中随机获取指定长度的片断。sample函数不会修改原有序列

  if len(selectedList) == 0:
    item = choice(nextnodeList)#random中的choice()是随机的返回序列中的一个
